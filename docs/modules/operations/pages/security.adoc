...
Each event includes an `event` field identifying the event type, along with category-specific fields.
The `msg` field also contains the event name (standard slog behavior).
...
----
{
  ...
  "level": "INFO",
  "msg": "authz.admin_override",
  "event": "authz.admin_override",
  "subject": "admin-user",
  "resource_type": "response",
  "resource_id": "resp_abc123",
  "resource_owner": "bob",
...
To separate audit logs from operational logs in your aggregation pipeline, filter on the `event` field.
