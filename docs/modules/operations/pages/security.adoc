...
Antwort supports multiple authentication methods, TLS termination through Kubernetes infrastructure, and multi-user isolation.
...
<3> Tenant identifier for multi-user isolation (user-group scoping within a single instance).
...
Antwort supports multi-user deployments through the `tenant_id` metadata field on the authenticated identity.
This provides user-group isolation within a single Antwort instance (not to be confused with multi-tenant isolation, which uses separate instances per tenant).
User-group isolation works as follows:
...
. The storage layer uses the tenant ID to scope response storage, ensuring that user groups cannot access each other's conversation history.
...
Antwort supports scope-based authorization that restricts API access based on user roles.
When the `auth.authorization.role_scopes` map is configured, each API endpoint requires a specific scope, and only users whose effective scopes include the required scope are allowed through.

Scopes use the `resource:action` pattern (e.g., `responses:create`, `vector_stores:read`).
The special scope `*` grants unrestricted access.

When `role_scopes` is empty or not configured, scope enforcement is disabled and all authenticated requests pass through to the next middleware layer.
This preserves full backward compatibility with existing deployments.
...
Effective scopes are computed as the union of:

. Direct scopes from the identity (JWT `scope` claim or API key scopes)
. Role-expanded scopes from the `role_scopes` mapping, based on the user's roles

For more details and examples, see the xref:tutorial:authorization.adoc[] tutorial.

== Resource Permissions

Vector stores and files support a Unix-style permissions model with three access levels: owner, group (same tenant), and others (cross-tenant).
Each level can grant read (`r`), write (`w`), and delete (`d`) permissions.

The permissions string format is `owner|group|others` (e.g., `rwd|r--|---`).
New resources default to `rwd|---|---` (owner-only access).

This model enables controlled resource sharing.
For example, an admin can create a shared vector store with group-read permissions (`rwd|r--|---`), allowing all users in the same tenant to search it while restricting modifications to the owner.

See the xref:tutorial:authorization.adoc[] tutorial for configuration examples.

== Resource Ownership

Antwort enforces per-user resource ownership for responses, conversations, and vector stores.
Every resource created by an authenticated user is tagged with the user's `Identity.Subject` as the owner.
Users can only access resources they own, with the following exceptions:

* **Admin override**: Users with the configured admin role can read and delete any resource within their tenant.
Admin users cannot modify (write to or add items to) resources owned by other users.
* **Backward compatibility**: Resources with an empty owner field (created before ownership was enabled) are accessible to all authenticated users within the same tenant.
* **No authentication**: When no authenticator is configured (`auth.type: none`), ownership checks are skipped entirely.

Ownership denials always return HTTP 404 (not 403) to prevent information leakage about resource existence.

=== Configuring Admin Access

To enable admin override, configure the admin role name and the JWT roles claim:
...
auth:
  ...
  jwt:
    jwks_url: https://idp.example.com/.well-known/jwks.json
    roles_claim: realm_access.roles    # <1>
  authorization:
    admin_role: antwort-admin          # <2>
----
<1> Dot-path to the JWT claim containing user roles.
For Keycloak, the default `realm_access.roles` works out of the box.
<2> The role name that grants admin access within a tenant.

With API key authentication, set the admin role in the key's metadata:
...
auth:
  ...
  authorization:
    admin_role: admin
  api_keys:
    - key_file: /run/secrets/admin-key
      subject: admin-user
      tenant_id: org-acme
      roles: admin                     # <1>
----
<1> Comma-separated list of roles for this API key user.

=== Ownership Debug Logging

When a user is denied access to a resource due to ownership, Antwort logs the denial at DEBUG level.
...
[source,text]
----
level=DEBUG msg="ownership denied" subject=user-2 resource_id=resp_abc123 operation=GetResponse
----
...
Audit logging records security-relevant events and state-changing operations separately from operational logs.
It is disabled by default and designed to be nil-safe, meaning zero overhead when turned off.
When enabled, audit events are emitted through a dedicated `slog.Logger` instance that writes to a configurable output destination.

=== Configuration

Enable audit logging through the configuration file or environment variables.
...
audit:
  ...
  format: json         # <2>
  output: stdout       # <3>
  file: /var/log/antwort/audit.log  # <4>
----
<1> Enable audit event emission. Default: `false`.
<2> Output format: `json` (structured, machine-readable) or `text` (human-readable). Default: `json`.
<3> Output destination: `stdout` or `file`. Default: `stdout`.
<4> File path for audit logs. Required when `output` is `file`.

Equivalent environment variables:

[source,bash]
----
export ANTWORT_AUDIT_ENABLED=true
export ANTWORT_AUDIT_FORMAT=json
export ANTWORT_AUDIT_OUTPUT=stdout
----

=== Event Catalog

Antwort emits 12 audit event types, organized into five categories.
Each event includes a `audit.event` field identifying the event type, along with category-specific fields.
...
| `auth.success`
| INFO
| `auth_method`, `remote_addr`

| `auth.failure`
| WARN
| `auth_method`, `remote_addr`, `error`
...
| `authz.scope_denied`
| WARN
| `endpoint`, `required_scope`, `effective_scopes`

| `authz.ownership_denied`
| INFO
| `resource_type`, `resource_id`, `operation`

| `authz.admin_override`
| INFO
| `resource_type`, `resource_id`, `resource_owner`, `operation`
|===
...
| INFO
| `resource_type`, `resource_id`, `old_permissions`, `new_permissions`
|===
...
| `config.startup`
| INFO
| `auth_enabled`, `audit_enabled`, `role_count`, `scope_enforcement`
|===

=== Example Output

A successful authentication event in JSON format:
...
----
{
  "time": "2026-03-04T10:15:31Z",
  "level": "INFO",
  "msg": "audit",
  "audit.event": "resource.created",
  "resource_type": "response",
  "resource_id": "resp_abc123",
  "subject": "alice"
}
----

An admin override event:

[source,json]
----
{
  "time": "2026-03-04T10:16:00Z",
  "level": "INFO",
  "msg": "audit",
  "audit.event": "authz.admin_override",
  "resource_type": "response",
  "resource_id": "resp_abc123",
  "resource_owner": "bob",
...
When `audit.output` is set to `stdout` (the default), audit log entries are written to standard output alongside operational logs.
In Kubernetes, DaemonSet-based log agents (such as Fluentd, Fluent Bit, or Vector) automatically collect stdout output from all containers without any custom configuration.

To separate audit logs from operational logs in your aggregation pipeline, filter on the `audit.event` field.
...
condition = 'exists(.audit.event)'
----

When `audit.output` is set to `file`, audit logs are written to a dedicated file at the path specified by `audit.file`.
Mount a persistent volume or emptyDir at the log directory to ensure logs survive Pod restarts.

=== Troubleshooting

**No audit output visible**::
Verify that `audit.enabled` is set to `true`.
Check the effective configuration by looking for the `config.startup` event in the operational logs, which reports whether audit logging is active.

**File permission errors**::
When using file output, ensure the Antwort process has write permissions to the directory specified in `audit.file`.
In Kubernetes, use an `emptyDir` volume or a PersistentVolumeClaim with appropriate access modes.

**High log volume**::
Audit logging emits one entry per security-relevant event.
In high-traffic deployments, consider routing audit logs to a dedicated file and configuring log rotation, or use a log aggregation pipeline with sampling.
