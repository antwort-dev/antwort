...
API key and JWT/OIDC authentication with tenant isolation.
Each tenant's responses are stored separately, and cross-tenant access is prevented at the storage layer.
...
* xref:architecture.adoc[Architecture] provides a deep dive into the request flow, component layers, and extension points.
* xref:api-reference.adoc[API Reference] documents the complete wire format, including request schemas, response schemas, SSE event types, and error codes.
* xref:quickstarts:index.adoc[Quickstarts] walk through deploying Antwort on Kubernetes, from a minimal single-Pod setup to production configurations with PostgreSQL and multi-tenant auth.
* xref:extensions:overview.adoc[Extensions] explains how to implement custom providers, tool executors, authenticators, and storage backends.
* xref:operations:monitoring.adoc[Operations] covers production deployment, monitoring, and troubleshooting.
