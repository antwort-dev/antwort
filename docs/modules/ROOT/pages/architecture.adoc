...
* *PostgreSQL* -- production-grade persistence using `pgx/v5`. Supports multi-tenant isolation, where each tenant's data is separated at the query level.
...
The xref:extensions:overview.adoc[Extensions] module provides detailed guidance on implementing each one.
