...
The key `"tenant_id"` has special meaning: when present, the auth middleware calls `storage.SetTenant()` to inject it into the context, enabling multi-tenant storage scoping.
