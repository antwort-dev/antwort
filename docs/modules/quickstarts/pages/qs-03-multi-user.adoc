...
:description: Deploy antwort with Keycloak for JWT-based authentication and tenant isolation.

Deploy antwort with Keycloak for JWT-based authentication and tenant isolation.
Two users (alice and bob) are pre-configured in separate tenants, demonstrating that each user can only access their own data.
