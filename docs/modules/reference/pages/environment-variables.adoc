...
| `ANTWORT_MODE`
| `engine.mode`
| Server mode: `gateway`, `worker`, or `integrated`. Default: `integrated`.
...
| `audit.format`
| Audit log format: `json` or `text`. Default: `json`.

| `ANTWORT_AUDIT_OUTPUT`
| `audit.output`
| Audit log destination: `stdout` or `file`. Default: `stdout`.

| `ANTWORT_AUDIT_FILE`
| `audit.file`
| File path for audit log output. Required when `ANTWORT_AUDIT_OUTPUT` is `file`.
...
| `ANTWORT_DEBUG`
| `logging.debug`
| Comma-separated debug categories (e.g., `providers,engine`). Enables category-specific debug logging at DEBUG level.

| `ANTWORT_RESILIENCE_ENABLED`
| `resilience.enabled`
| Set to `true` to enable circuit breaker and retry for backend communication.

| `ANTWORT_RESILIENCE_FAILURE_THRESHOLD`
| `resilience.failure_threshold`
| Parsed as an integer. Consecutive failures before circuit opens. Default: `5`.

| `ANTWORT_RESILIENCE_MAX_ATTEMPTS`
| `resilience.max_attempts`
| Parsed as an integer. Total request attempts (1 = no retry). Default: `3`.
|===
